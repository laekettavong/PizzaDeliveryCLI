        ...
        this.fetchOrders(hrsOffset, (err, orderData) => {
            if(!err && orderData && orderData.length > 0){
                ...
                orderData.forEach((order) => {
                    ...
                    let line = `ID: \x1b[33m${order.id}\x1b[0m   Customer: ${order.email}  Total: \x1b[33m${order.total}\x1b[0m  Date: ${createDate.toDateString()} ${createDate.toTimeString()}`;
                    console.log(line);
                });
        ...
        this.fetchOrders(hrsOffset, (err, orderData) => {
            if(!err && orderData && orderData.length > 0) {
                ...
                highestOrders.forEach((order) => {
                    console.log(`Highest order: \x1b[33m${order.id}\x1b[0m   User: ${order.email}   Amount: \x1b[33m${order.total}\x1b[0m`);
                });
                CliUtils.verticalSpace();
            }
        });
        ...
        this.fetchOrders(hrsOffset, (err, orderData) => {
            if(!err && orderData && orderData.length > 0) {
                ...
                lowestOrders.forEach((order) => {
                    console.log(`Lowest order: \x1b[33m${order.id}\x1b[0m   User: ${order.email}   Amount: \x1b[33m${order.total}\x1b[0m `);
                });
                CliUtils.verticalSpace();
            }
        });
    ...
    static displayOrdersMedian(hrsOffset) {
        this.fetchOrders(hrsOffset, (err, orderData) => {
            if(!err && orderData && orderData.length > 0) {
                ...
                console.log(`Median total: \x1b[33m${Currency.USD(totalAmount)}\x1b[0m for \x1b[33m${orderData.length}\x1b[0m orders`);
            }
        });
    }
