...
const OrdersServices = class OrderServices {
    ...
    */
    static processOrder(reqData, callback) {
        ...
        let authToken = reqData.headers.token ? StringHelper.isValidString(reqData.headers.token) : StringHelper.isValidString(reqData.payload.token);
        if(stripeToken){
            UserContext.getContext(authToken, (err, context) => {
                if(!err && context) {
                        FsHelper.read(Constants.CARTS, context.user.shoppingcart, (err, cartData) => {
                            if(!err && cartData) {
                                cartData.date = new Date();
                                cartData.id = context.user.shoppingcart;
